...
def main():
    ...
    if choice == "ログイン":
        ...
        new_region = st.text_input("地域")
        new_company_name = st.text_input("会社名")
        new_name = st.text_input("氏名")
        new_phone_number = st.text_input("電話番号")
        ...
        if st.button("登録"):
            if new_username in users:
                ...
                users[new_username] = {'password': hash_password(new_password), 'region': new_region, 'company_name': new_company_name, 'name': new_name, 'phone_number': new_phone_number, 'email': new_email}
